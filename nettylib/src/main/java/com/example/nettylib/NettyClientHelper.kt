...
import com.example.nettylib.proto.HeartBeatData.HeartBeat
...
import com.example.nettylib.websocket.WebSocketProtoBufClient
...
open class NettyClientHelper(val context: Context) {
    ...
    private val tcpProtoBufClient = object : WebSocketProtoBufClient() {
        override fun onReadData(
            channelHandlerContext: ChannelHandlerContext,
            data: MessageLite
        ) {
    ...
    private fun ssl() {
        val clientContextFromAssets = SecureChatSslContextFactory.getClientContextFromAssets(
            context,
            "bksclient.keystore",
            "bksclient.keystore"
        )
    ...
    fun send(str: String) {
        val newBuilder = HeartBeat.newBuilder()
        newBuilder.time = 1000000L
        val build = newBuilder.build()
        //ch.writeAndFlush(frame)
        //Log.i("NettyClientHelper", "frame:$frame")
        tcpProtoBufClient.send(build)
    }
